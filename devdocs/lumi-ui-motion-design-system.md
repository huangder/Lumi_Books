...
- 首页滑到最顶后继续下拉，在上方留白中淡入「松手以继续阅读最近书籍」：提示锚定状态栏下方 `8dp` 居中，透明度与下拉进度同步，并从上方 `12dp` 滑入；松手时内容下移达到 `96dp`（约手指 `240dp`）才打开继续阅读卡片当前那本书，未达阈值或推回不触发。下拉阻尼保持既有越界回弹参数不变，不做触顶吸附或追加阻力。
...
- 编辑模式在原位替换顶栏，不新增一层导航。
- 长按菜单锚定书籍触点出现并保留玻璃材质。
- 长按封面/文件夹的浮层按 `ContextMenuMotion` 统一入场：信息面板立即（`0ms`）以 `spring(0.75 / 420)` 进入、操作面板 `60ms` 后以 `spring(0.72 / 380)` 进入，被按封面 `spring(0.62 / 420)` 放大到 `1.08`，遮罩 `180ms` 压暗；退场为 `spring(0.88 / 320)`、封面与遮罩 `260ms` 归位。宫格、列表、分类页共用同一套参数，不得只调其中一处。
- 关闭长按菜单必须只触发一次状态清理，不能重复 dismiss 或访问已释放锚点。
- 「分类与标签」页「自定义分类」按层级缩进，父文件夹行左侧保留 `34dp` 折叠按钮位（叶子分类用同宽占位对齐图标），箭头折叠时旋转 `-90°`、`160ms` 完成；折叠只收起该行的后代，行内书籍数量仍按整棵子树统计，点按行本身继续进入分类，折叠状态随页面重建保留。
...
- 应用内打开书籍使用 Apple Books 风格的 Shared Element / Hero 转场：封面作为共享锚点，沿轻微 Quadratic Bezier 路径移动到 Reader，位置 `600ms`、尺寸 `650ms`、圆角独立收敛；封面保持原宽高比，并依次通过主题色底、上下渐隐过渡、封面图像、主题色底完成融合，不使用拉伸、bounce 或 overshoot。
- 书库在转场期间轻微 blur/dim 并降低视觉权重；Reader 保持轻量 Shell，正文在窗口建立并加载完成后淡入。
- 返回时先等待书库完成排序与重新布局，再按该书当前的封面 bounds 反向落位；阅读进度/下载进度等封面覆盖物随共享窗口一起运动，卡片不可见时使用普通 fade/scale fallback。外部入口、电子墨水与减少动态效果仍使用原加载页或无动画路径。
...
- 按钮下拉菜单先短促压缩到 `90%`，再连续生长并越过目标尺寸、回弹归位。预收缩占弹簧进度 `0–0.28`，原控件图层随尺寸一起压缩；不增加停顿。使用 `liquidGlassMenuAnchor` 接管绘制，原控件保留布局，退出落回后才恢复原绘制。
- 独立按钮沿近侧边展开，圆角由控件自身形状过渡到 `24dp`；原控件图层在生长进度 `0–0.3` 内交接到菜单表面。屏幕安全区、窗口坐标和最大高度由当前菜单宿主统一处理。
- 菜单内容在进度 `0.15–0.85` 内淡入并由模糊变清晰，始终按目标宽度测量，不缩放文字；内容尚未成形或正在退出时不接受选项操作。Android 31 以下省略内容模糊。
- TTS 语速、音调、定时使用嵌入式触发源，从对应局部区域向上展开；共享播放胶囊固定为 `56dp`，保留播放区域的点击，长菜单在可用高度内滚动。
- 玻璃采样层必须包含页面底色，阅读器包含纸色，避免透明采样在折射后出现内部矩形色块。浮层使用窗口物理坐标定位，RTL 仅改变起止边语义。
- Lumi、Material 3、Liquid Glass 共用形变，材质和强制实色设置各自保留；减少动态效果与电子墨水使用短淡变。书库长按菜单（书籍封面与分类文件夹）走 `ui/bookshelf/ContextMenuMotion.kt` 的统一参数，不再使用旧的 `stiffness 145` + `80/190ms` 分步延迟（那样入场要 700ms 以上，长按后发粘）。
...
|---|---|
| 按压反馈 | `120ms` |
| 长按菜单入场（书库封面 / 文件夹） | 信息面板 `spring(0.75 / 420)` 立即、操作面板 `+60ms`（`≈130ms` / `≈220ms` 到 95%，约 `490ms` 完全落定） |
| 长按菜单退场 | 面板 `spring(0.88 / 320)`，封面与遮罩 `260ms` |
| 按钮菜单形变 | 可中断弹簧：进入 `stiffness 150 / damping 0.62`（约 8% 进度过冲），关闭 `220 / 0.80`；放慢蓄力与展开，反向接管保留当前进度和速度 |
| 按钮菜单绘制交接 | 短促压缩至 `90%` 后生长，生长进度 `0–0.3` 从原控件图层过渡到菜单表面；退出完成后释放源图层 |
| 菜单内容浮现 | 进度 `0.15–0.85` 淡入和去模糊，文字保持原始尺寸 |
| 减少动态菜单 | `100ms` 淡变，无尺寸、位置和模糊动画 |
...
锚定菜单（`ui/components/AnchoredLiquidGlassMenu.kt`）的进入与退出不再使用固定时长：它由一组可中断的弹簧参数（`LiquidGlassMenuMotion` 的 stiffness / damping / initial velocity）驱动，默认值即上表。`LumiMotion.MenuEnterMillis/MenuExitMillis` 仍供阅读器等非锚定弹层使用，不要删除。
