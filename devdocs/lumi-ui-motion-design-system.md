...
- Liquid Glass 的开关、拖动条激活区使用当前 `AppColors.Accent`；分段选择 TagBar 按 6.5 节使用中性黑/白透明层。
...
- 返回时先等待书库完成排序与重新布局，再按该书当前的封面 bounds 反向落位；阅读进度/下载进度等封面覆盖物随共享窗口一起运动，卡片不可见时使用普通 fade/scale fallback。外部入口、电子墨水与减少动态效果仍使用原加载页或无动画路径；此外「设置 → 显示与外观 → 打开书籍过渡」可让用户在「一镜到底」（默认）与「加载页」之间显式选择，选择加载页时所有应用内入口都退回原加载页路径。
...
- 统计周期、两处笔记分类、赞助方式、阅读页边距目标、翻页/显示模式和 TXT 搜索范围统一使用 `LiquidGlassSegmentedControl`；独立 chips、单选卡片、WebDAV 按钮组和底部主导航不套用本节规则。
- 轨道采样页面背景，并以一个不可见捕获层组合轨道内容；外层轨道可以保留克制的 `0.5dp` 玻璃轮廓。
- 内部移动色块只使用黑/白透明叠层和 combined backdrop 折射：浅色模式叠半透明黑，深色模式叠半透明白，不使用主题强调色。色块在真实玻璃与 fallback 分支都禁止边框、描边、边缘高光和 inner shadow，只允许柔和投影提供层次。
- 色块静止时不施加透镜形变；按压或拖动时透镜最多为 `2dp / 4dp`，不使用 vibrancy 和色散，避免采样内容堆积在上下边缘。选中文字和图标使用 `TextPrimary` 的 `86%` 不透明度。
- 点击其他选项立即提交，色块使用 `spring(dampingRatio = 0.68, stiffness = 520)` 移动。到位后可越过目标再减速回弹，超调不超过相邻行程 `8%` 且不超过 `6dp`；轨道不得裁切边缘超调。
- 只有从当前移动色块按下才能开始拖动。横向超过 Touch Slop 后按中心位置 1:1 跟手；纵向先越过阈值时取消控件手势并交还页面滚动。
- 拖动期间最近的可用选项只更新色块与文字/图标预览，不切换业务内容；松手吸附后最多提交一次。disabled 项不能成为点击或吸附目标。
- 等宽与调用方指定宽度共用同一套锚点计算；动态选项必须重建锚点。RTL 下视觉位置、点击命中和拖动方向全部镜像。
- 每段提供 `Role.Tab` 与 selected 语义。视觉高度不足 `44dp` 时，触控和语义区域仍至少为 `44dp`。
- 减少动态效果时改为 `100ms` 短促吸附，并关闭超调、按压缩放、色散和速度形变。
...
| 分段选择 TagBar | `spring(0.68 / 520)`；超调 ≤ 相邻行程 `8%` 且 ≤ `6dp` |
